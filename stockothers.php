...
$sql = "SELECT p.id, p.name, COALESCE(ssq.quantity, 0) AS available_stock 
FROM products p
LEFT JOIN others_quantity ssq ON ssq.product_id = p.id
WHERE p.category_id = 5 OR p.category_id = 6 OR p.category_id = 8";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<p style='color: white'>" . $row["name"] . " - " . $row["available_stock"] . "</p>";

        // Add form to update quantity
        echo "<form style='color: white' method='post' action='update_quantityothers.php'>";
        ...
        echo "</form>";
        echo "<form method='get' action='insert_stockothers.php'>";
        echo "<input type='hidden' name='product_id' value='" . $row["id"] . "' />";
