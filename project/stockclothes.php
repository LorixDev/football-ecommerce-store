...
FROM products p
LEFT JOIN clothing_sizes_quantity ssq ON ssq.product_id = p.id
LEFT JOIN size_obleceni ss ON ss.id = ssq.size_id
WHERE p.category_id = 2 OR p.category_id = 3 OR p.category_id = 4 OR p.category_id = 7";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<form class='white' method='post' action='update_quantityclothes.php'>";
        ...
        echo "</form>";
        echo "<form method='get' action='insert_stockclothes.php'>";
        echo "<input type='hidden' name='product_id' value='" . $row["id"] . "' />";
        echo "<input type='hidden' name='size_id' value='" . $row["size_id"] . "' />";
